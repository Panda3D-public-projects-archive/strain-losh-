...
class Interface(DirectObject.DirectObject):
    ...
    def __init__(self, ge):
        ...
        self.unit_move_orientation = globals.HEADING_NONE
    ...
    def loadTurnArrows(self, dest):
        ...
        for i in xrange(9):
            ...
            if i == 0:
                ...
                h = 45
                key = globals.HEADING_NW
            elif i == 1:
                pos = Point3(x, y+delta, height)
                h = 0
                key = globals.HEADING_N                
            elif i ==2:
                pos = Point3(x+delta, y+delta, height)
                h = -45
                key = globals.HEADING_NE                
            elif i ==3:
                pos = Point3(x-delta, y, height)
                h = 90
                key = globals.HEADING_W                
            elif i ==4:
                pos = Point3(x+delta, y, height)
                h = -90
                key = globals.HEADING_E                
            if i == 5:
                pos = Point3(x-delta, y-delta, height)
                h = 135
                key = globals.HEADING_SW                
            elif i == 6:
                pos = Point3(x, y-delta, height)
                h = 180
                key = globals.HEADING_S                
            elif i ==7:
                pos = Point3(x+delta, y-delta, height)
                h = 225               
                key = globals.HEADING_SE
            elif i == 8:
                pos = Point3(x, y, height)
                h = 0
                key = globals.HEADING_NONE
            m.setPos(pos)
            m.setH(h)
    ...
    def mouseLeftClickUp(self):
        ...
        """
        if self.selected_unit and self.unit_move_destination and self.unit_move_orientation != globals.HEADING_NONE:   
            # Send movement message to engine
            x = self.unit_move_destination.getX()
            y = self.unit_move_destination.getY()
            if self.unit_move_orientation == globals.HEADING_NW:
                o = Point2(x-1, y+1)
            elif self.unit_move_orientation == globals.HEADING_N:
                o = Point2(x, y+1)
            elif self.unit_move_orientation == globals.HEADING_NE:
                o = Point2(x+1, y+1)
            elif self.unit_move_orientation == globals.HEADING_W:
                o = Point2(x-1, y)
            elif self.unit_move_orientation == globals.HEADING_E:
                o = Point2(x+1, y)
            elif self.unit_move_orientation == globals.HEADING_SW:
                o = Point2(x-1, y-1)
            elif self.unit_move_orientation == globals.HEADING_S:
                o = Point2(x, y-1)
            elif self.unit_move_orientation == globals.HEADING_SE:
                o = Point2(x+1, y-1)
