...
class UnitMarkerRenderer():
    ...
    def setSelected(self, unit_id):
        ...
        self.parent.shader_manager.clearShader(unit.model)
        unit.isHovered = False
        unit.marker.setTexture(self.tex_selected)
    ...
    def refreshTargetInfo(self, unit_id):
        if self.enemy_markers.has_key(unit_id):
            if self.parent.parent.turn_player == self.parent.parent.player_id and self.parent.parent.sel_unit_id != None:
                # Delete previous OnScreenText node
                for child in self.enemy_markers[unit_id]['info_node'].getChildren():
                    child.removeNode()
    ...
    def setMarker(self, unit_id):
        if self.enemy_markers.has_key(unit_id):
            ...
            if self.parent.unit_renderer_dict.has_key(unit_id): 
                ...
                self.parent.unit_renderer_dict[unit_id].marker.reparentTo(self.parent.unit_renderer_dict[unit_id].node)
        else:
            cm = CardMaker('')
            ...
            self.enemy_markers[unit_id]['info_node'].setPos(0.05, 0, -0.05)
            ...
            self.refreshTargetInfo(unit_id)
            
            self.parent.unit_renderer_dict[unit_id].marker.setTexture(self.tex_enemy)
            self.parent.unit_renderer_dict[unit_id].marker.reparentTo(self.parent.unit_renderer_dict[unit_id].node)
    ...
    def clearMarker(self, unit_id):
        if self.enemy_markers.has_key(unit_id):
            ...
            if self.parent.unit_renderer_dict.has_key(unit_id):
                self.parent.unit_renderer_dict[unit_id].marker.detachNode()  
